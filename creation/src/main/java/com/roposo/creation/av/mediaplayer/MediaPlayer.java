...
import static com.roposo.creation.av.mediaplayer.MediaPlayerUtils.*;

/**
 * @author maguggen on 04.06.2014.
 */
public class MediaPlayer implements MediaPlayerImpl {
    ...
    private MediaExtractorCallback mMediaExtractionListener;
    ...
    @Override
    public void setMediaExtractionListener(MediaExtractorCallback extractorCallback) {
        mMediaExtractionListener = extractorCallback;
    }
    ...
    @Override
    public void setRealTimeExtract(boolean realTimeExtract) {
    }
    ...
    public void setDataSource(MediaSource source, int videoTrackIndex, int audioTrackIndex)
        ...
        if (mVideoTrackIndex != MediaCodecDecoder.INDEX_NONE) {
            ...
            if (VERBOSE) Log.d(TAG, "selected video track #" + mVideoTrackIndex + " " + mVideoFormat.toString());
        }
        ...
        if (mAudioTrackIndex != MediaCodecDecoder.INDEX_NONE) {
            ...
            if (VERBOSE) Log.d(TAG, "selected audio track #" + mAudioTrackIndex + " " + mAudioFormat.toString());
        }
    ...
    private int getTrackIndex(MediaExtractor mediaExtractor, String mimeType) {
        ...
        for (int i = 0; i < mediaExtractor.getTrackCount(); ++i) {
            ...
            if (VERBOSE) Log.d(TAG, format.toString());
    ...
    private void prepareInternal() throws IOException, IllegalStateException {
        MediaCodecDecoder.OnDecoderEventListener decoderEventListener = new MediaCodecDecoder.OnDecoderEventListener() {
            ...
            @Override
            public void onAudioFrameRendered(MediaCodecDecoder.FrameInfo frameInfo, long offsetus) {
                MediaCodec.BufferInfo bufferInfo = new MediaCodec.BufferInfo();
                bufferInfo.set(frameInfo.offset, frameInfo.size, frameInfo.presentationTimeUs,
                        frameInfo.endOfStream ? MediaCodec.BUFFER_FLAG_END_OF_STREAM : 0);
                if (mMediaExtractionListener != null) {
                    mMediaExtractionListener.sendAudioFrame(frameInfo.data, bufferInfo, frameInfo
                            .endOfStream);
                }
            }
        };
        ...
        if (mAudioTrackIndex != MediaCodecDecoder.INDEX_NONE) {
            ...
            try {
                ...
                MediaCodecDecoder ad = new MediaCodecAudioDecoder(mAudioExtractor != null ? mAudioExtractor : mVideoExtractor,
                        passive, mAudioTrackIndex, decoderEventListener, mAudioPlayback, mPlayAudio);
        ...
        if (true) {
            mDecoders.setClipSize(mBeginTime, mEndTime);
            if (mDecoders.getVideoDecoder() != null) {
    ...
    public void prepareAsync() throws IllegalStateException {
        if (mCurrentState == State.PREPARING || mCurrentState == State.PREPARED) return;
        if (mCurrentState != State.INITIALIZED && mCurrentState != State.STOPPED) {
            throw new IllegalStateException("current state is " + mCurrentState.name());
        }
    ...
    /**
     * @see android.media.MediaPlayer#setSurface(Surface)
     */
    public void setSurface(Surface surface) {
        ...
        if (mPlaybackThread == null) {
            // Player not prepared yet, so we can set the timing mode
            setVideoRenderTimingMode(VideoRenderTimingMode.AUTO); // the surface could be a GL texture, so we switch to sleep timing mode
            updateSurfaceScreenOn();
        } else {
            // Player is already prepared, just change the surface
            mPlaybackThread.setSurface(mSurface);
    ...
    public void pause() {
        if (mCurrentState != State.PREPARED) {
            mCurrentState = State.ERROR;
//            Toast.makeText(ContextHelper.getContext(), "MediaPlayer: pause: entered error state", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    public void seekTo(long usec) {
        ...
        if (VERBOSE) Log.d(TAG, "seekTo " + usec + " with video sample offset " + mVideoMinPTS);
    ...
    public boolean isPrepared() {
        return mPlaybackThread != null && mCurrentState == State.PREPARED;
    }
        ...
        if (mPlaybackThread != null) {
            ...
            synchronized (mReleaseSyncLock) {
                try {
                    ...
                    mReleaseSyncLock.wait(150);
                    // TODO muddassiriqbal sahilbajaj jugaad here. will see later, if it causes side-effects
    ...
    public long getCurrentPosition() {
         ...
         * while in exact mode). */
        return (mSeeking ? mSeekTargetTime : mCurrentPosition);
    }
    ...
    void setVideoRenderTimingMode(VideoRenderTimingMode mode) {
        ...
        if (VERBOSE) Log.d(TAG, "setVideoRenderTimingMode " + mode);
    ...
    public void setClipSize(long startTime, long endTime) {
        ...
        mEndTime = endTime;
        if (mDecoders != null && !mDecoders.isEmpty()) {
            mDecoders.setClipSize(mBeginTime, mEndTime);
        }
        if (VERBOSE) Log.d(TAG, "Start time: " + startTime + " End time: " + mEndTime);
    }

    private void initialSeek() throws IOException {
        if (mBeginTime > 0) {
            mDecoders.seekTo(SeekMode.FAST_TO_CLOSEST_SYNC, mBeginTime);
        } else {
            mDecoders.seekTo(SeekMode.FAST_TO_PREVIOUS_SYNC, mBeginTime);
        }
    }
    ...
    /**
     * Register a callback to be invoked when the progress of the player
     * has changed.
     *
     * @param listener the callback that will be run.
     */
    ...
    enum VideoRenderTimingMode {
        ...
        /**
         * Renders frames as fast as possible. Receiver end is responsible to handle frames accordingly.
         * To be used for recording where the frame render time has no meaning,
         * just the timestamps which are going to be embedded in the video stream.
         */
        RECORD;
    ...
    private class PlaybackThread extends HandlerThread implements Handler.Callback {
        ...
        public synchronized void start() {
            ...
            if (VERBOSE) Log.d(TAG, "PlaybackThread started");
        }
            ...
            try {
                ...
                switch (msg.what) {
                    ...
                    case DECODER_SET_SURFACE:
                        ...
                        return true;
                    default:
                        if (VERBOSE) Log.d(TAG, "unknown/invalid message");
                        return false;
                }
            } catch (InterruptedException e) {
                if (VERBOSE) Log.d(TAG, "decoder interrupted", e);
        ...
        private void playInternal() throws IOException, InterruptedException {
            if (mDecoders.isEOS() || mCurrentPosition >= mEndTime) {
                mCurrentPosition = mBeginTime;
                if (VERBOSE) Log.d(TAG, "play Current position = " + mCurrentPosition);
                initialSeek();
            }
        ...
        private void loopInternal() throws IOException, InterruptedException {
            ...
            if (mBuffering && cachedDuration > -1 && cachedDuration < BUFFER_LOW_WATER_MARK_US && !mDecoders.hasCacheReachedEndOfStream()) {
                //if (VERBOSE) Log.d(TAG, "buffering... " + mDecoders.getCachedDuration() + " / " + BUFFER_LOW_WATER_MARK_US);
            ...
            if (VERBOSE) Log.d(TAG, "update Current position = " + mCurrentPosition);
            mEventHandler.sendMessage(mEventHandler.obtainMessage(MEDIA_PROGRESS,
                    (int) (mCurrentPosition >> 32), (int) mCurrentPosition));
        ...
        private void playbackLoop(int delay) {
        ...
        private void seekInternal(long usec) throws IOException, InterruptedException {
            ...
            if (!newSeekWaiting) {
                ...
                mCurrentPosition = mDecoders.getCurrentDecodingPTS();
                if (VERBOSE) Log.d(TAG, "Seek Current position = " + mCurrentPosition);
                mEventHandler.sendMessage(mEventHandler.obtainMessage(MEDIA_PROGRESS,
                        (int) (mCurrentPosition >> 32), (int) mCurrentPosition));
                mSeeking = false;
                mAVLocked = false;
        ...
        private void releaseInternal() {
            ...
            if (VERBOSE) Log.d(TAG, "PlaybackThread destroyed");
        ...
        private void renderVideoFrame(MediaCodecDecoder.FrameInfo videoFrameInfo) throws InterruptedException {
            ...
            if (VERBOSE) Log.d(TAG, "VPTS " + mCurrentPosition
                    + " APTS " + (mAudioPlayback != null ? mAudioPlayback.getCurrentPresentationTimeUs() : 0)
            ...
            if (waitingTime < -1000) {
                ...
                if (VERBOSE) Log.d(TAG, "LAGGING " + waitingTime);
                mEventHandler.sendMessage(mEventHandler.obtainMessage(MEDIA_INFO,
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MEDIA_SET_VIDEO_SIZE:
                    if (VERBOSE) Log.d(TAG, "onVideoSizeChanged");
                ...
                case MEDIA_ERROR:
                    ...
                    return;
                case MEDIA_INFO:
                    if (VERBOSE) Log.d(TAG, "onInfo");
                    if (mOnInfoListener != null) {
                        mOnInfoListener.onInfo(MediaPlayer.this, msg.arg1, msg.arg2);
                    }
                    return;
                case MEDIA_BUFFERING_UPDATE:
                    //if (VERBOSE) Log.d(TAG, "onBufferingUpdate");
                    if (mOnBufferingUpdateListener != null)
                        mOnBufferingUpdateListener.onBufferingUpdate(MediaPlayer.this, msg.arg1);
                    return;
                case MEDIA_PROGRESS:
                    long currentTime = (((long) msg.arg1) << 32) | ((long) msg.arg2 & 0xffffffffL);
                    if (mOnProgressListener != null) {
                        mOnProgressListener.onProgress(MediaPlayer.this, currentTime);
                    }
                    break;
                default:
                    // nothing

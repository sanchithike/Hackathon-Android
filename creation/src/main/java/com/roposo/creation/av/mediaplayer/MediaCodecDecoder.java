...
abstract class MediaCodecDecoder {
    ...
    public static final boolean VERBOSE = false || MediaPlayer.VERBOSE;
    protected String TAG = MediaCodecDecoder.class.getSimpleName();
    OnDecoderEventListener mOnDecoderEventListener;
    ...
    protected final void reinitCodec() throws IOException {
        try {
            ...
            if (!mFormat.containsKey(MediaFormat.KEY_BIT_RATE)) {
                mFormat.setInteger(MediaFormat.KEY_BIT_RATE, 3000 * 1000);
            }
            Log.d(TAG, "trying to configure decoder for format: " + mFormat);
    ...
    public final FrameInfo dequeueDecodedFrame() throws IOException {
        ...
        long dpt = getCurrentDecodingPTS();
        mOutputEos = dpt >= mEndTime || (res >= 0 && (mBufferInfo.flags & MediaCodec
        ...
        if (mOutputEos && mRepresentationChanging) {
            ...
            fi.offset = mBufferInfo.offset;
            fi.size = mBufferInfo.size;
            ...
            if (VERBOSE) Log.d(TAG, "output format has changed to " + format);
            onOutputFormatChanged(format);
        } else if (res == MediaCodec.INFO_TRY_AGAIN_LATER) {
            if (VERBOSE) Log.d(TAG, "dequeueOutputBuffer timed out");
        }
    ...
    protected FrameInfo seekTo(MediaPlayer.SeekMode seekMode, long seekTargetTimeUs,
        ...
        if (VERBOSE) Log.d(TAG, "seeking to:                 " + seekTargetTimeUs);
        if (VERBOSE) Log.d(TAG, "extractor current position: " + extractor.getSampleTime());

        extractor.seekTo(seekTargetTimeUs, seekMode.getBaseSeekMode());

        if (VERBOSE) Log.d(TAG, "extractor new position:     " + extractor.getSampleTime());
    ...
    public void release() {
        mCodec.stop();
        mCodec.release();
        if (VERBOSE) Log.d(TAG, "decoder released");
    }

    public void setClipSize(long beginTime, long endTime) {
    ...
    interface OnDecoderEventListener {
        ...
        void onAudioFrameRendered(FrameInfo frameInfo, long offsetus);
    }
